...
TEST="nb_ants"
echo "Running test $TEST"
declare -a arr_ants=("40" "60")
declare -a RUNS
for nb_ants in "${arr_ants[@]}"
do
    echo "$nb_ants ants"
    f1=$(python3 run.py --runs $TOT_RUNS --dataset graph3 --iterations 3000 --ants $nb_ants --evaporation 0.1 --alpha 1.1 --beta 1 --save-dir $SAVEDIR --sub-dir $TEST --jobs $JOBS | tail -1)
    RUNS=("${RUNS[@]}" "$f1")
done
echo "${RUNS[@]}"
python3 plots/plot_fitness.py --paths "${RUNS[@]}"
...
# do
#     echo "Evaporation = $evap"
#     f1=$(python3 run.py --runs $TOT_RUNS --dataset graph3 --iterations 3000 --ants 40 --evaporation $evap --alpha 1.1 --beta 1 --save-dir $SAVEDIR --sub-dir $TEST --jobs $JOBS | tail -1)
#     RUNS=("${RUNS[@]}" "$f1")
# done
# python3 plots/plot_fitness.py --paths "${RUNS[@]}"

echo "Running best hyper"
f1=$(python3 run.py --runs $TOT_RUNS --dataset graph3 --iterations 3000 --ants 60 --evaporation 0.05 --alpha 1.1 --beta 1 --save-dir $SAVEDIR --sub-dir best_run --jobs $JOBS | tail -1)
python3 plots/plot_fitness.py --paths $f1